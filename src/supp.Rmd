...
\begin{align*}
    D_{KL}(\calR \parallel \hat{\calR}) &= D_{KL}(f_0(y;\eta,\calR) \parallel f_1(y;\eta,\hat{\calR})) \\
    &= D_{KL}\left(\prod_{t=1}^n f_0(y_t;\eta_t,\calR_t) \parallel \prod_{t=1}^n f_1(y_t;\eta_t,\hat{\calR}_t)\right)\\
    &= \sum_{t=1}^n D_{KL}\left(f_0(y_t;\eta_t,\calR_t) \parallel f_1(y_t;\eta_t,\hat{\calR}_t)\right),\text{ ($y_t$ are independent, conditional on $\calR_t,\eta_t$)}\\
    &= \sum_{t=1}^n \exp(\log(\eta_t\hat{\calR}_t)) - \exp(\log(\eta_t\calR_t)) - \exp(\log(\eta_t\calR_t))\log\frac{\eta_t\hat{\calR}_t}{\eta_t\hat{\calR}_t}, \text{ (definition of Bregman divergence)} \\
    &= \sum_{t=1}^n \eta_t\hat{\calR}_t) - \eta_t\calR_t) - \log\frac{\hat{\calR}_t}{\hat{\calR}_t} \eta_t\calR_t\\
...
```{r display-time-long, echo = FALSE, fig.height = 5, fig.width = 8, fig.cap = "Running time comparison of all methods for long (measles and SARS) epidemics across all cases. Y-axis is on a logarithmic scale."}
