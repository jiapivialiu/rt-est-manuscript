...
then, the KL divergence between them can be written interms of the Bregman 
...
pair of SI parameters, e.g., we use SI of \sars to solve \meas epidemics,
and SI of \meas to solve short \flu epidemics. While, in the mild SI 
...
Figure \@ref(fig:display-KL-monthly-long) compares \EpiEstim with _monthly_
...
```{r display-KL-weekly-flu, echo=FALSE, fig.height=4, fig.width=6.2, fig.cap = "The mean KL divergence excluding the first week for flu epidemics, since EpiEstim with the weekly sliding window does not provide estimates for the first week. Y-axes are on a logarithmic scale."}
...
```{r display-KL-weekly-mis-si-measles, echo=FALSE, fig.height=6.5, fig.width=6.2, fig.cap = "The mean KL divergence excluding the first week for measles epidemics with SI misspecification, since EpiEstim with the weekly sliding window does not provide estimates for the first week. Y-axes are on a logarithmic scale."}
...
```{r display-KL-monthly-mis-si-measles, echo=FALSE, fig.height=6.5, fig.width=6.2, fig.cap = "The mean KL divergence excluding the first month for measles epidemics with SI misspecification, since EpiEstim with the monthly sliding window does not provide estimates for the first month. Y-axes are on a logarithmic scale."}
#Rt_res_long_month <- Rt_result %>% 
...
```{r display-KL-weekly-mis-si-flu, echo=FALSE, fig.height=6, fig.width=6.2, fig.cap = "The mean KL divergence excluding the first week for flu epidemics with SI misspecification, since EpiEstim with the weekly sliding window does not provide estimates for the first week. Y-axes are on a logarithmic scale."}
...
```{r display-KL-monthly-mis-si-flu, echo=FALSE, fig.height=6, fig.width=6.2, fig.cap = "The mean KL divergence excluding the first month for flu epidemics with SI misspecification, since EpiEstim with the monthly sliding window does not provide estimates for the first month. Y-axes are on a logarithmic scale."}
...
```{r display-time-flu-all, echo = FALSE, fig.height = 4.5, fig.width = 6.6, fig.cap = "Time comparisons of methods for short (flu) epidemics across all pairs of SI parameters per incidence distribution. Y-axes are on a logarithmic scale."}
...
```{r ci-example-measles-pois, echo = FALSE, fig.height=7, fig.width=6.5, fig.cap = "Example measles epidemics with Poisson incidence. Y-axes beyond 5 are truncated for a better illustration of small values."}
...
We measure the coverage of 95\% confidence intervals by three metrics: 
1-percentage of coverage per coordinate (if available) across all synthetic data, 
2-percentage of overall coverage for all available coordinates, and
3-interval score [@Bracher_2021] averaged over all available coordinates.

The first metric provides a vector of percentage of coverage across all 
synthetic data for each setting and method. The vector length may be shorter than
the length of epidemic, because, for example, \EpiEstim with weekly sliding window 
does not provide estimates for the first week. The second metric provides a 
percentage of coverage over all timepoints with available estimation for each 
synthetic data and method. 
...
$\mathbf{1}_{X}$ is the indicator function of the condition $X$. Here $n$ is the 
length of the timepoints with available estimation. For example, estimation in 
the first week is unavailable using \EpiEstim with weekly sliding window. 
...
```{r ci-coverage-measles, echo=FALSE, fig.cap="Percentage of CI coverage per coordinate across 50 synthetic measles epidemics.", fig.height=5, fig.width=6.5, message=FALSE, warning=FALSE}
Rt_res_ci <- CI_result %>%
...
```{r ci-coverage-sars, echo=FALSE, fig.cap="Percentage of CI coverage per coordinate across 50 synthetic SARS epidemics.", fig.height=5, fig.width=6.5, message=FALSE, warning=FALSE}
...
over 50 random \meas and \sars epidemics respectively. 
...
```{r ci-percentage-measles, echo=FALSE, fig.cap="Percentages of CI coverage over all timepoints for 50 synthetic measles epidemics. The orange dashed line represents 95\\% percentage of coverage across all timepoints.", fig.height=4.5, fig.width=6.2, message=FALSE, warning=FALSE}
...
```{r ci-percentage-sars, echo=FALSE, fig.cap="Percentages of CI coverage over all timepoints for 50 synthetic SARS epidemics. The orange dashed line represents 95\\% percentage of coverage across all timepoints.", fig.height=4.5, fig.width=6.2, message=FALSE, warning=FALSE}
...
interval scores of 95% CI averaged over 50 random \meas and \sars epidemics 
...
1), and the scores of \RtEstim are always around 1. \EpiLPS has large interval 
scores due to its large estimates at the early stage of the epidemic. Its large 
estimates (much larger than the true values) are multiplied by 
$\frac{2}{\alpha},\alpha=0.05$ when computing the interval score, so it 
yields extremely large interval scores.
...
```{r display-example-pois-sars, echo=FALSE, fig.height=4, fig.width=6.2, fig.cap = "Example of instantaneous reproduction number estimation for SARS epidemics with Poisson observations. Y-axes beyond 5 are truncated for a better illustration of small values."}
...
```{r  display-example-nb-meas, echo=FALSE, fig.height=4, fig.width=6.2, fig.cap = "Example of instantaneous reproduction number estimation for measles epidemics with negative binomial observations. Y-axes beyond 5 are truncated for a better illustration of small values."}
...
```{r display-example-pois-meas-alter, echo=FALSE, fig.height=4, fig.width=6.2, fig.cap = "Difference between the true instantaneous reproduction number and its estimation for measles epidemics with Poisson observations. Y-axes beyond 1.5 are truncated for a better illustration of small values."}
...
```{r display-example-pois-sars-alter, echo=FALSE, fig.height=4, fig.width=6.2, fig.cap = "Difference between the true instantaneous reproduction number and its estimation for SARS epidemics with Poisson observations. Y-axes beyond 1.5 are truncated for a better illustration of small values."}
fig_res_pois_meas_alter <- res_dat_long %>%
  filter(si_type == "SARS", dist == "Poisson") %>% 
  mutate(dRt = Rt_value - trueRt) %>%
  group_by(Rt_case, method) %>%
...
```{r display-example-nb-meas-alter, echo=FALSE, fig.height=4, fig.width=6.2, fig.cap = "Difference between the true instantaneous reproduction number and its estimation for measles epidemics with negative binomial observations. Y-axes beyond 1.5 are truncated for a better illustration of small values."}
fig_res_nb_sars_alter <- res_dat_long %>%
  filter(si_type == "measles", dist == "Negative Binomial") %>% 
  mutate(dRt = Rt_value - trueRt) %>%
  group_by(Rt_case, method) %>%
