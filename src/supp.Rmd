...
a long \meas epidemic, it takes almost 2 hours on the 
...
```{r NB-overdispersion, echo=FALSE, fig.height=4.4, fig.width=6, fig.cap = "Dispersion level of incidence of sample negative binomial epidemics."}
...
settings. \RtEstim\ is reasonably robust to misspecification of SI parameters: 
...
from negative binomial distribution, where \EpiLPS\ has a clear advantage, ignoring the large
...
\clearpage
## Experimental settings for coverage comparisons of confidence intervals
...
for every time point, for example, \EpiEstim with weekly sliding window 
...
\EpiLPS does the best here with averged coverage at all timepoints close to 1. 
...
Our method \RtEstim with degrees $k=1,2,3$ has 100\% coverage at most timepoints except the
...
Our method \RtEstim with $k=0$ tends to produce overly narrow intervals, leading to lower coverage. 
...
narrow intervals. \EpiFilter has lower percentages of coverage
...
```{r ci-coverage-measles, echo=FALSE, fig.cap="Percent of CI coverage per coordinate across 50 synthetic measles epidemics.", fig.height=5, fig.width=6.5, message=FALSE, warning=FALSE}
Rt_res_ci <- CI_result %>%
...
```{r ci-coverage-sars, echo=FALSE, fig.cap="Percent of CI coverage per coordinate across 50 synthetic SARS epidemics.", fig.height=5, fig.width=6.5, message=FALSE, warning=FALSE}
