...
incidence) averaged per coordinate across our \RtEstim and several alternative 
...
methods in total. We consider two lengths of epidemics with $n=50, 300$ 
timepoints respectively. Since \EpiNow2 runs too long (specifically, for 
one long epidemic, it takes ???TBA), we only compare it with other methods 
for short epidemics. 

We consider serial interval (SI) distributions of measles and SARS to generate 
long synthetic epidemics, and flu for short epidemics. Incident cases in 
...
$20000$ at the peak overall). We specifically consider a reasonably large 
overdispersion level of negative Binomial incidence is of size 5. Figure \@ref(fig:NB-overdispersion) displays the ratio of standard deviation over 
mean (called, sigma to mean ratio) of incidence across different settings 
using the same set of sample epidemics in the manuscript. Compared to the 
counterpart of Poisson incidence (which decreases quickly to be under 0.25 
and close to 0 at most timepoints) per $\calR_t$ scenario for each epidemic, 
the negative Binomial incidence appears to have an apparently larger sigma to 
mean ratio (staying at around 0.5 at most timepoints) implying a distinguishable 
overdispersion level. 

```{r NB-overdispersion, echo=FALSE, fig.height=4.4, fig.width=6, fig.cap = "Dispersion level of incidence of sample epidemics", fig.pos="!ht"}
...
distributions. The misspecification of serial interval distributions are either 
mild or major, where, in the major misspecification, we use SI of SARS to solve 
long measles epidemics and SI of measles to solve short flu epidemics. While, in 
the mild SI misspecification, we consider a shaped (mean increased by 2) and 
scaled (standard deviation increased by $10%$) parameters for both short flu 
and long measles epidemics, denoted as \flus and \meass respectively. These 
...
for short epidemics. Figure \@ref(fig:si-dist) displays the SI distributions 
(\meas, \meass, \sars, \flu, and \flus) used in the experiments. 

```{r si-dist, echo=FALSE, fig.height=4.4, fig.width=6.2, fig.cap = "Density curves of serial interval distributions used in the experiments", fig.pos="!ht"}
dat_si <- data.table(
  x = 0:40,
...
Table \@ref(tab:df-exp) summarizes the aforementioned experimental setting for accuracy comparison. Poisson and negative Binomial (NB) distributions for incidence and 4 $\calR_t$ scenarios are used for all settings. Each experimental setting is replicated for $50$ times, which yields $12800$ experiments with long epidemics and $2700$ with short epidemics. 
...
1/2000, and use the smoothed $\calR_t$ as estimates. 
...
We visualize the selected key results of accuracy comparison using long synthetic 
epidemics in Section 3.2 in the manuscript. Other main experimental results are 
displayed as follows.

# Supplementary experimental results on accuracy comparison

Figures \@ref(fig:display-KL-monthly-measles) and \@ref(fig:display-KL-monthly-sars) 
display the KL divergence excluding the first months for measles and SARS 
epidemics respectively. To compare \EpiEstim with ***monthly*** sliding windows 
with other methods, we average the KL divergence per coordinate excluding the 
timepoints in the first months for all approaches, since \EpiEstim estimates with 
the monthly sliding windows are not available until the second months. 
...
Rt_result <- readRDS(here::here("dat/Rt_result_combined.RDS")) #rt_cluster_results611.RDS
Rt_result <- Rt_result %>%
...
```{r display-KL-monthly-measles, echo=FALSE, fig.height=5, fig.width=6.2, fig.cap = "KL divergence excluding the first months for measles epidemics. Y-axis is on a logarithmic scale.", fig.pos="!ht"}
...
```{r display-KL-monthly-sars, echo=FALSE, fig.height=5, fig.width=6.2, fig.cap = "KL divergence excluding the first months for SARS epidemics. Y-axis is on a logarithmic scale.", fig.pos="!ht"}
...
display the KL divergence for short epidemics averaged per coordinate excluding 
the first weeks and months resepctively. 
...
Figures x and x display KL divergence (excluding first weeks and months 
respectively) for all 8 methods with shaped and scaled \meas SI parameters with 
long \meas epidemics across all settings. 

```{r display-KL-weekly-measles-measless, echo=FALSE, fig.height=5, fig.width=6.2, fig.cap = "KL divergence excluding the first weeks for measle epidemics with mild SI misspecification. Y-axis is on a logarithmic scale."}
...
```{r display-KL-monthly-measles-measless, echo=FALSE, fig.height=5, fig.width=6.2, fig.cap = "KL divergence excluding the first months for measle epidemics with mild SI misspecification. Y-axis is on a logarithmic scale."}
Rt_res_long_month_no_outliers <- Rt_result %>% 
...
Figures x and x display KL divergence (excluding first weeks and months 
respectively) for all 9 methods with shaped and scaled \flu SI parameters 
with short \flu epidemics across all settings. 

```{r display-KL-weekly-flu-fluss, echo=FALSE, fig.height=5, fig.width=6.2, fig.cap = "KL divergence excluding the first weeks for flu epidemics with mild SI misspecification. Y-axis is on a logarithmic scale."}
...
```{r display-KL-monthly-flu-fluss, echo=FALSE, fig.height=5, fig.width=6.2, fig.cap = "KL divergence excluding the first weeks for flu epidemics with mild SI misspecification. Y-axis is on a logarithmic scale."}
...
Figures \@ref(fig:display-KL-weekly-measles-sars) and \@ref(fig:display-KL-monthly-measles-sars)
display KL divergence (excluding first weeks and months respectively) for all 8 
methods with \sars SI parameters with long \meas epidemics across all settings. 

```{r display-KL-weekly-measles-sars, echo=FALSE, fig.height=5, fig.width=6.2, fig.cap = "KL divergence excluding the first weeks for measle epidemics with major SI misspecification. Y-axis is on a logarithmic scale."}
...
```{r display-KL-monthly-measles-sars, echo=FALSE, fig.height=5, fig.width=6.2, fig.cap = "KL divergence excluding the first months for measle epidemics with major SI misspecification. Y-axis is on a logarithmic scale."}
...
Figures \@ref(fig:display-KL-weekly-flu-measles) and \@ref(fig:display-KL-monthly-flu-measles) display KL divergence (excluding first weeks and months respectively) for all 9 
methods with \meas SI parameters with short \flu epidemics across all settings. 

```{r display-KL-weekly-flu-measles, echo=FALSE, fig.height=4, fig.width=6.2, fig.cap = "KL divergence excluding the first weeks for flu epidemics with major SI misspecification. Y-axis is on a logarithmic scale.", fig.pos="!ht"}
...
```{r display-KL-monthly-flu-measles, echo=FALSE, fig.height=4, fig.width=6.2, fig.cap = "KL divergence excluding the first months for flu epidemics with major SI misspecification. Y-axis is on a logarithmic scale.", fig.pos="!ht"}
...
## Display estimates and confidence intervals for sample epidemics

Let's review the estimated $\calR_t$ with 95% confidence intervals for the sample 
long epidemics by all methods in the manuscript in Figure xx.
...
```{r ci-example-measles-pois, echo = FALSE, fig.height=7, fig.width=6.5, fig.cap = "Fitted Rt with 95% confidence intervals for measles epidemics with Poisson incidence."}
...
```{r  ci-example-measles-nb, echo = FALSE, fig.height=7, fig.width=6.5, fig.cap = "Fitted Rt with 95% confidence intervals for measles epidemics with negative Binomial incidence."}
...
```{r  ci-example-sars-pois, echo = FALSE, fig.height=7, fig.width=6.5, fig.cap = "Fitted Rt with 95% confidence intervals for SARS epidemics with negative Binomial incidence."}
fig_res_full_nb_measles <- res_dat_long %>%
...
```{r  ci-example-sars-nb, echo = FALSE, fig.height=7, fig.width=6.5, fig.cap = "Fitted Rt with 95% confidence intervals for SARS epidemics with nengative Binomial incidence."}
fig_res_full_nb_measles <- res_dat_long %>%
...
```{r ci-percentage-measles, echo=FALSE, fig.cap="Averaged percentages of 95% CI with measles epidemics.", fig.height=4.5, fig.width=6.2, message=FALSE, warning=FALSE}
...
```{r ci-percentage-sars, echo=FALSE, fig.cap="Averaged percentages of 95% CI with SARS epidemics.", fig.height=4.5, fig.width=6.2, message=FALSE, warning=FALSE}
...
```{r ci-score-measles, echo=FALSE, fig.cap="Averaged interval scores of 95% CI with measles epidemics.", fig.height=4.5, fig.width=6.2, message=FALSE, warning=FALSE}
...
```{r ci-score-sars, echo=FALSE, fig.cap="Averaged interval scores of 95% CI with SARS epidemics.", fig.height=4.5, fig.width=6.2, message=FALSE, warning=FALSE}
...
# use estimated Covid-19 serial interval distributions
covid_si_pars <- c(3,3) #shape, scale
len <- nrow(cases)

# EpiFilter: cannot incorporate missing values
...
fit_covidBC_RtEstim1 <- cv_estimate_rt(
  ...
  nsol = 50, error_measure = "deviance", lambda_min_ratio = 1e-6, dist_gamma = covid_si_pars)
...
# EpiEstim (weekly)
Dmax <- floor(stats::qgamma(0.9999, covid_si_pars[1], scale = covid_si_pars[2]))
prob_gamma <- discretize_gamma(1:Dmax, shape = covid_si_pars[1], scale = covid_si_pars[2])
datBC <- cases
colnames(datBC) <- c("dates", "I")
## weekly
t_start_week <- seq(2, len - 6)
t_end_week <- t_start_week + 6
config_week <- make_config(list(si_distr = c(0, prob_gamma), t_start = t_start_week, t_end = t_end_week))
...
config_month <- make_config(list(si_distr = c(0, prob_gamma), t_start = t_start_month, t_end = t_end_month))
fit_month <- EpiEstim::estimate_R(incid = datBC, config = config_month, method = "non_parametric_si")
...
# EpiLPS
fitLPS <- EpiLPS::estimR(incidence = cases$Cases, si = prob_gamma, K = 40, 
