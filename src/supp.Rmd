...
\begin{equation*}
  \begin{split}
    &\sum_{y_1=0}^{\infty}\frac{(\eta_1\calR_1)^{y_1}e^{-\eta_1\calR_1}}{y_1!} 
      \Big(y_1\log \frac{\calR_1}{\hat{\calR}_1} + \sum_{t=2}^n y_t\log \frac{\calR_t}{\hat{\calR}_t} - 
      \sum_{t=1}^n \eta_t(\calR_t - \hat{\calR}_t) \Big) \\
    = & \lr{\sum_{y_1=0}^{\infty}\frac{(\eta_1\calR_1)^{y_1-1}e^{-\eta_1\calR_1}}{(y_1-1)!} 
...
We compare the accuracy of the estimated effective reproduction numbers using 
the mean Kullback Leibler (KL) divergence (with Poisson distributional assumption 
on incidence) in (10) across our \RtEstim and several alternative 
...
a long \meas epidemic, it takes almost 2 hours (115 minutes computed on 
Cedar cluster provided by Compute Canada), we only compare it with other methods 
for short epidemics. 

We consider serial interval (SI) distributions of \meas and \sars to generate 
long synthetic epidemics, and \flu for short epidemics, inspired by @cori2013new. 
The means and standard deviations of SI distributions are estimated by existing literatures; specifically, $(14.9, 3.9)$ for \meas (@groendyke2011bayesian), 
$(8.4,3.8)$ for \sars (@lipsitch2003transmission), and $(2.6,1.5)$ for \flu (@ferguson2005strategies, @boelle2011transmission). 
...
epidemics in Section 3.2. Other main experimental results are displayed in 
...
```{r display-KL-weekly-long, echo=FALSE, fig.height=6, fig.width=6.2, fig.cap = "Mean KL divergence excluding the first weeks for measles and SARS epidemics, since EpiEstim with the weekly sliding window does not provide estimates for the first week. Y-axis is on a logarithmic scale."}
...
fig_kl_week_long <- Rt_res_long_week %>% #
  ...
  labs(x = "Method", y = "Mean KL divergence (excluding the first weeks)") +
  theme_bw() +
...
```{r display-KL-monthly-long, echo=FALSE, fig.height=6, fig.width=6.2, fig.cap = "Mean KL divergence excluding the first months for measles and SARS epidemics, since EpiEstim with the monthly sliding window does not provide estimates for the first month. Y-axis is on a logarithmic scale."}
...
fig_kl_month_long <- Rt_res_long_month %>% #
  ...
  labs(x = "Method", y = "Mean KL divergence (excluding the first months)") +
  theme_bw() +
  theme(axis.text.x = element_blank(), legend.position = "bottom") +
  guides(color = guide_legend(title = NULL, byrow=TRUE))
...
```{r display-KL-weekly-flu, echo=FALSE, fig.height=3.5, fig.width=6.2, fig.cap = "Mean KL divergence excluding the first weeks for flu epidemics, since EpiEstim with the weekly sliding window does not provide estimates for the first week. Y-axis is on a logarithmic scale."}
...
```{r display-KL-monthly-flu, echo=FALSE, fig.height=3.5, fig.width=6.2, fig.cap = "Mean KL divergence excluding the first months for flu epidemics, since EpiEstim with the monthly sliding window does not provide estimates for the first month. Y-axis is on a logarithmic scale."}
...
```{r display-KL-weekly-mis-si-measles, echo=FALSE, fig.height=6, fig.width=6.2, fig.cap = "Mean KL divergence excluding the first weeks for measles epidemics with SI misspecification, since EpiEstim with the weekly sliding window does not provide estimates for the first week. Y-axis is on a logarithmic scale."}
...
```{r display-KL-monthly-mis-si-measles, echo=FALSE, fig.height=6, fig.width=6.2, fig.cap = "Mean KL divergence excluding the first months for measles epidemics with SI misspecification, since EpiEstim with the monthly sliding window does not provide estimates for the first month. Y-axis is on a logarithmic scale."}
#Rt_res_long_month <- Rt_result %>% 
...
```{r display-KL-weekly-mis-si-flu, echo=FALSE, fig.height=6, fig.width=6.2, fig.cap = "Mean KL divergence excluding the first weeks for flu epidemics with SI misspecification, since EpiEstim with the weekly sliding window does not provide estimates for the first week. Y-axis is on a logarithmic scale."}
...
```{r display-KL-monthly-mis-si-flu, echo=FALSE, fig.height=6, fig.width=6.2, fig.cap = "Mean KL divergence excluding the first months for flu epidemics with SI misspecification, since EpiEstim with the monthly sliding window does not provide estimates for the first month. Y-axis is on a logarithmic scale."}
...
for long (measles and SARS) epidemics. 
\EpiEstim with both weekly and monthly sliding windows are very fast and converge 
in less than 0.1 seconds. Piecewise constant \RtEstim (with k=0) estimates can 
be generated within 0.1 seconds as well. \EpiLPS is slightly slower, but still 
very fast and within 1 second for all experiments. 
Piecewise linear and cubic \RtEstim (with $k=1$ and $k=3$ respectively) are 
slower, but mostly within 10 seconds. 
...
```{r display-time-long, echo = FALSE, fig.height = 5, fig.width = 5, fig.cap = "Time comparisons of all methods for long (measles and SARS) epidemics across all cases. Y-axis is on a logarithmic scale."}
...
We visualize the running times of each case in separate panels in Figures \@ref(fig:display-time-meas) and \@ref(fig:display-time-sars) for measles and 
SARS epidemics respectively. Similar results as in Figure \@ref(fig:display-time-long) 
are found in each setting. 

```{r display-time-meas, echo = FALSE, fig.height = 7.5, fig.width = 6.6, fig.cap = "Time comparisons of all methods for measles epidemics with each choice of SI parameter for modelling per incidence distribution per Rt scenario (excluding outliers for better illustration). Y-axis is on a logarithmic scale."}
runtime_epi_meas <- Rt_result_runtime_no_outlier %>%
  filter(len != 50, si_type == "measles") |>
  group_by(dist, Rt_case, method) %>%
  ggplot(aes(x = method, y = runtime / 1e6)) +
  geom_boxplot(aes(col = method)) +
...
```{r display-time-sars, echo = FALSE, fig.height = 4.5, fig.width = 6.6, fig.cap = "Time comparisons of all methods for SARS epidemics with each choice of SI parameter for modelling per incidence distribution per Rt scenario (excluding outliers for better illustration). Y-axis is on a logarithmic scale."}
runtime_epi_sars <- Rt_result_runtime_no_outlier %>%
...
Figure \@ref(fig:display-time-flu-all) displays the running times of all methods for
short (flu) epidemics. Figure \@ref(fig:display-time-flu) displays the running 
times for each setting separately, and finds similar results as in the overall 
...
```{r display-time-flu, echo = FALSE, fig.height = 7, fig.width = 6.6, fig.cap = "Time comparisons of methods for short (flu) epidemics with different cases in different panels. Y-axis is on a logarithmic scale."}
...
are visualized in Figures \@ref(fig:ci-example-measles-nb) and \@ref(fig:ci-example-sars-pois) as follows.
...
We output a vector of the CI coverage for each timepoint per experiment, 
the percentage of coverage of all timepoints, and the interval score 
$$score_{\alpha}(\calR,u,l) = \frac{1}{n} \sum_{t=1}^n (u_t-l_t) + \frac{2}{\alpha}(l_t-\calR_t)\boldsymbol{1}_{(\calR_t<l_t)} + \frac{2}{\alpha}(\calR_t-u_t)\boldsymbol{1}_{(\calR_t>u_t)}$$, where $\alpha=0.05$ is the 
significance level, $l,u$ are the lower and upper bounds, $\calR_t$ is the true 
effective reproduction number, and $\mathbf{1}_{X}$ is the indicator function 
of the condition $X$. [@Bracher_2021].
...
# fit model
fit_week <- EpiEstim::estimate_R(incid = rtestim::cancovid$incident_cases, config = config_week, 
...
# fit model
fit_month <- EpiEstim::estimate_R(incid = rtestim::cancovid$incident_cases, config = config_month, 
                                  method = "non_parametric_si")
...
```{r plot-cancovid-ci, echo=FALSE, fig.height=8, fig.width=6.2, fig.cap = "Rt estimates with CIs for Covid19."}
...
```{r plot-cancovid-one, echo=FALSE, fig.height=3.5, fig.width=6.2, fig.cap = "Rt estimates for Covid19."}
dat_cancovid_fitted_long %>%
  filter(fitted_Rt > 0) %>% # remove NAs marked by -1
  group_by(Method) %>%
  ggplot(aes(x = Date)) + 
...
fig_inc_fit <- data.table(inc_dat, variant = can_pred_class$var) %>%
  ...
  scale_x_date(date_breaks = "8 months", date_labels = "%Y-%m-%d", expand = expansion(c(0, 0.05))) +
  labs(x = "Date", y = "Fitted incidence (in 1,000)") +
  scale_y_continuous(expand = expansion(c(0, 0.05))) + 
  geom_vline(xintercept = start_dates$`min(date)`[-2], linetype = "dashed") +
  theme_bw()
...
ggsave(here::here("fig/covid_full_res.png"), CanCovid_Rt_all, width = 6.5, height = 5)
```
