...
# Experimental comparisons between EpiEstim with monthly sliding windows and other methods for Section 3.1
...
```{r display-KL-monthly-full-res, echo=FALSE, fig.height=5.5, fig.width=6.2, fig.cap = "Fig 10. KL divergence for EpiEstim with monthly sliding windows and other methods. Y-axis is on a logarithmic scale."}
...
res_dat2 <- res_dat %>%
  select(
    time, Rt2, Pois_Epi_week2, Pois_Rtestim2, Pois_Rtestim22, Pois_EpiLPS2
  ) %>%
  pivot_longer(!time, names_to = "Rt_type", values_to = "Rt_value") %>%
  mutate(Rt_type = fct_recode(Rt_type,
    "True Rt" = "Rt2",
...
res_dat3 <- res_dat %>%
  select(
    time, Rt3, Pois_Epi_week3, Pois_Rtestim3, Pois_EpiLPS3
  ) %>%
  pivot_longer(!time, names_to = "Rt_type", values_to = "Rt_value") %>%
  mutate(Rt_type = fct_recode(Rt_type,
    "True Rt" = "Rt3",
...
res_dat4 <- res_dat %>%
  select(
    time, Rt4, Pois_Epi_week4, Pois_Rtestim4, Pois_EpiLPS4
  ) %>%
  pivot_longer(!time, names_to = "Rt_type", values_to = "Rt_value") %>%
  mutate(Rt_type = fct_recode(Rt_type,
    "True Rt" = "Rt4",
...
res_dat_long <- rbind(res_dat1, res_dat2, res_dat3, res_dat4)
res_dat_long$Rt_case = as.factor(res_dat_long$Rt_case)
res_dat_long$Rt_type = as.factor(res_dat_long$Rt_type)

fig_Pois_res <- res_dat_long %>% 
  mutate(Rt_case = fct_recode(Rt_case,
    "Scenario 1: piecewise constant" = "1",
    "Scenario 2: piecewise exponential" = "2",
    "Scenario 3: piecewise linear" = "3",
    "Scenario 4: periodic" = "4"
  )) %>%
  group_by(Rt_case, Rt_type) %>%
  ggplot(aes(x = time, y = Rt_value, col = Rt_type)) +
  geom_line(na.rm = TRUE) +
  facet_wrap(~ Rt_case, scales = "free_y") + 
  scale_colour_manual(values = cbPalette2) +
  labs(y = "Rt estimates for Poisson incidence", color = "", x = "", title = "") +
  theme_bw() +
  theme(legend.position = "bottom") 
print(fig_Pois_res)
```
...
res_dat_n2 <- res_dat %>%
  select(time, Rt2, NB_Epi_week2, NB_Rtestim2, NB_Rtestim22, NB_EpiLPS2) %>%
  pivot_longer(!time, names_to = "Rt_type", values_to = "Rt_value") %>%
  mutate(Rt_type = fct_recode(Rt_type,
...
res_dat_nb_long <- rbind(res_dat_n1, res_dat_n2, res_dat_n3, res_dat_n4)
res_dat_nb_long$Rt_case <- as.factor(res_dat_nb_long$Rt_case)
res_dat_nb_long$Rt_type <- as.factor(res_dat_nb_long$Rt_type)

fig_NB_res <- res_dat_nb_long %>%
  mutate(Rt_case = fct_recode(Rt_case,
    "Scenario 1: piecewise constant" = "1",
    "Scenario 2: piecewise exponential" = "2",
    "Scenario 3: piecewise linear" = "3",
  ...
  facet_wrap(~ Rt_case, scales = "free_y") + 
  scale_colour_manual(values = cbPalette2) +
  labs(y = "Rt estimates for negative Binomial incidence", color = "", x = "", title = "") +
  theme_bw() +
  theme(legend.position = "bottom") 
print(fig_NB_res)
```
