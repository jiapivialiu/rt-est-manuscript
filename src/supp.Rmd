...
#knitr::opts_chunk$set(fig.cap.prefix = "Fig ")
library(rtestim)
library(EpiEstim)
library(EpiLPS)
library(data.table)
...
divergence for $\exp$, e.g. @WainwrightJordan2008,
\begin{align*}
...
# Additional details on experimental settings
...
\RtEstim and several alternative 
methods, including \EpiEstim with weekly and monthly sliding windows, \EpiLPS, 
\EpiFilter, \EpiNow2, and \RtEstim with degrees k=0,1,2,3, resulting in $9$ 
methods in total. We consider two durations of epidemics with $n=50$ or $n=300$ 
timepoints respectively. Since \EpiNow2 may take too long to converge (for example,
a long \meas epidemic requires nearly 2 hours to converge on the 
...
We use the serial interval (SI) distributions of \meas and \sars to generate 
...
Incident cases in synthetic \meas epidemics are relatively low (generally $<1000$ at 
the peak), while \sars incident cases are relatively large (between $15000$ 
...
using the same set of sample epidemics in Figs 5 and 6, and all figures in 
...
```{r NB-overdispersion, echo=FALSE, fig.height=4.4, fig.width=6, fig.cap = "Dispersion level of incidence for sample negative binomial epidemics."}
...
dat_example_NB_SMR <- dat_example %>%
  ...
  mutate(smr = (mean + mean^2 / size) / mean)
dat_example_pois_SMR <- dat_example %>%
  filter(dist == "Poisson") %>%
  group_by(Rt_case, si_type) %>%
  mutate(smr = mean / mean)
dat_example_SMR <- rbind(dat_example_NB_SMR)
...
fig_smr <- dat_example_SMR %>%
  ...
  labs(y = "Variance / mean", x = "Time") + 
  scale_x_continuous(expand = expansion(c(0, 0.05))) + 
  scale_y_log10(expand = expansion(c(0, 0.05))) + 
  theme_bw() +
...
pair of SI parameters, e.g., we use the SI of \sars for generated \meas epidemics,
and the SI of \meas for generated \flu epidemics. In the mild SI 
...
(for epidemic generating, model fitting): (\meas, \meas), (\sars, \sars), 
(\meas, \meass), (\meas, \sars) for long epidemics and (\flu, \flu), 
(\flu, \flus), (\flu, \meas) for short epidemics. Fig \@ref(fig:si-dist) 
displays all SI distributions (\meas, \meass, \sars, \flu, and \flus) used in 
the experiments. 

```{r si-dist, echo=FALSE, fig.height=4.4, fig.width=6.2, fig.cap = "Probability densities of serial interval distributions used in the experiments."}
dat_si <- data.table(
  x = 0:40,
...
df_exp <- data.table(
  ...
  si_pars = c("measles, adj-measles, SARS", "SARS", "flu, adj-flu, measles"),
...
\clearpage
# Additional accuracy comparisons {#sec:exp-accuracy}
...
We have displayed the accuracy of all methods (where \EpiEstim uses a weekly 
sliding window) for \meas and \sars sample epidemics using KL divergence 
excluding the first week since \EpiEstim does not provide estimates in the first 
week in Figs 3 and 4 in the manuscript, where we exclude the outliers. 
A full visualization including the outliers is in Fig 
\@ref(fig:display-KL-weekly-long). 

Fig \@ref(fig:display-KL-monthly-long) compares \EpiEstim with _monthly_
sliding windows with other methods. We average the KL divergence per coordinate 
excluding the timepoints in the first month for all approaches, since \EpiEstim 
...
\EpiEstim with longer sliding windows assumes similarity of neighbouring $\calR_t$ 
across longer periods, and thus, is smoother and less accurate compared to the 
one with shorter sliding windows. 

```{r display-KL-weekly-long, echo=FALSE, fig.height=6, fig.width=6.2, fig.cap = "The mean KL divergence excluding the first week for measles and SARS epidemics, since EpiEstim with the weekly sliding window does not provide estimates for the first week. Y-axes are on a logarithmic scale."}
...
fig_kl_week_long <- Rt_res_long_week %>% #
  ...
  labs(x = "", y = "KL divergence (excluding the first week)") +
  theme_bw() +
...
```{r display-KL-monthly-long, echo=FALSE, fig.height=6, fig.width=6.2, fig.cap = "The mean KL divergence excluding the first month for measles and SARS epidemics, since EpiEstim with the monthly sliding window does not provide estimates for the first month. Y-axes are on a logarithmic scale."}
...
fig_kl_month_long <- Rt_res_long_month %>% #
  ...
  labs(x = "", y = "KL divergence (excluding the first month)") +
  theme_bw() +
  theme(axis.text.x = element_blank(), legend.position = "bottom",
        axis.ticks.x = element_blank()) +
  guides(color = guide_legend(title = NULL, byrow=TRUE))
...
Figs \@ref(fig:display-KL-weekly-flu) and \@ref(fig:display-KL-monthly-flu)
...
\RtEstim\ (with $k=0,1$) are the most accurate for Poisson incidence, \RtEstim 
($k=2,3$), \EpiLPS\ and \EpiFilter\ are accurate as well with median KL estimates around 1.
For negative binomial incidence, the advantage of \RtEstim\ is less obvious, 
but \RtEstim\ with all degrees still has the lowest median with a small IQR. 

```{r display-KL-weekly-flu, echo=FALSE, fig.height=4, fig.width=6.2, fig.cap = "The average KL divergence excluding the first week for flu epidemics, since EpiEstim with the weekly sliding window does not provide estimates for the first week. Y-axes are on a logarithmic scale."}
...
```{r display-KL-monthly-flu, echo=FALSE, fig.height=4, fig.width=6.2, fig.cap = "The mean KL divergence excluding the first month for flu epidemics, since EpiEstim with the monthly sliding window does not provide estimates for the first month. Y-axes are on a logarithmic scale."}
...
\clearpage
# Experimental results under misspecification of the serial interval distributions

## SI misspecification for long epidemics

Figs \@ref(fig:display-KL-weekly-mis-si-measles) and 
...
misspecification (using adjusted \meas SI parameters) and "major" 
misspecification (using \sars SI parameters) for long \meas epidemics across all 
settings. \RtEstim\ is reasonably robust to misspecification of the SI parameters: 
median KL error for each problem design is almost always the lowest with the lowest 
IQR. \EpiLPS\ is a strong competitor under negative binomial incidence since it
it uses the correct loss function. \EpiFilter\ is also
quite robust to SI misspecification under Poisson incidence. 

```{r display-KL-weekly-mis-si-measles, echo=FALSE, fig.height=6.5, fig.width=6.2, fig.cap = "The mean KL divergence excluding the first week for measles epidemics with SI misspecification. Y-axes are on a logarithmic scale."}
...
```{r display-KL-monthly-mis-si-measles, echo=FALSE, fig.height=6.5, fig.width=6.2, fig.cap = "The mean KL divergence excluding the first month for measles epidemics with SI misspecification. Y-axes are on a logarithmic scale."}
#Rt_res_long_month <- Rt_result %>% 
...
Figs \@ref(fig:display-KL-weekly-mis-si-flu) and 
...
misspecification (using slightly modified \flu SI parameters) and "major" 
...
\EpiLPS\ is less satisfactory when only the first 
...
```{r display-KL-weekly-mis-si-flu, echo=FALSE, fig.height=6, fig.width=6.2, fig.cap = "The mean KL divergence excluding the first week for flu epidemics with SI misspecification. Y-axes are on a logarithmic scale."}
...
fig_kl_short_week_si <- Rt_res_short_week_mis_si %>%
  ...
  stat_boxplot(aes(col = method)) +
  facet_grid(mis_type * dist ~ Rt_case, scales = "free") +
  scale_y_log10(breaks = c(0.1, 1, 10, 100, 1000, 1e4L), labels = c(0.1, 1, 10, expression(10^2), expression(10^3), expression(10^4))) +
  scale_colour_manual(values = cbPalette[-2]) +
  labs(x = "", y = "KL divergence (excluding the first week)") +
  theme_bw() +
  theme(axis.text.x = element_blank(), legend.position = "bottom",
...
```{r display-KL-monthly-mis-si-flu, echo=FALSE, fig.height=6, fig.width=6.2, fig.cap = "The mean KL divergence excluding the first month for flu epidemics with SI misspecification. Y-axes are on a logarithmic scale."}
...
\EpiEstim with both sliding windows is very fast and converges in less than 0.1 
...
within 1 second for all experiments. \EpiFilter\ is typically similar to our method
with $k>0$. Piecewise linear and cubic \RtEstim (with 
...
separate panel in Figs \@ref(fig:display-time-meas) and 
...
It is remarkable that \RtEstim computes 50 lambda values with 10-fold CV for 
...
```{r display-time-long, echo = FALSE, fig.height = 5, fig.width = 6.6, fig.cap = "Running time comparison of all methods for long (measles and SARS) epidemics across all cases. Y-axis is on a logarithmic scale."}
runtime_epi_long <- Rt_result %>% # rtestim with k=1,2,3 sometimes runs much slower
  ...
  scale_y_log10(breaks = c(0.1, 1, 10, 100, 1000), labels = c(0.1, 1, 10, expression(10^2), expression(10^3))) +
  labs(y = "Running time in seconds", x = "") +
  theme_bw() +
...
```{r display-time-meas, echo = FALSE, fig.height = 7.5, fig.width = 6.6, fig.cap = "Running time comparison of all methods for measles epidemics with each pair of SI parameters (measles, adjusted measles, and SARS; excluding outliers for better illustration). Y-axes are on a logarithmic scale."}
...
```{r display-time-sars, echo = FALSE, fig.height = 4.5, fig.width = 6.6, fig.cap = "Running time comparison of all methods for SARS epidemics with each choice of SI distribution. Y-axes are on a logarithmic scale."}
...
for short (\flu) epidemics. All methods except \EpiNow2 converge within 
...
```{r display-time-flu-all, echo = FALSE, fig.height = 4.5, fig.width = 6.6, fig.cap = "Time comparisons of methods for short (flu) epidemics across all pairs of SI distribution. Y-axes are on a logarithmic scale."}
...
## Estimates and confidence intervals for sample epidemics {#sec:samples}

Figs 5 and 6 in the manuscript provided $\calR_t$ estimates by all methods on 
sample \meas epidemics with Poisson incidence and \sars epidemics with negative 
binomial incidence respectively. Figs \@ref(fig:ci-example-measles-pois) and 
...
for other settings are shown in Figs \@ref(fig:ci-example-measles-nb) and 
...
Under negative binomial incidence, \RtEstim with $k=0$ fails to recover the
...
scenarios, except in the case of the periodic $\calR_t$ curve with large negative binomial incidence, where \EpiLPS\ has a clear advantage ignoring the inflated estimates at the early stage. 
...
```{r ci-example-measles-pois, echo = FALSE, fig.height=7, fig.width=6.5, fig.cap = "Example measles epidemics with Poisson incidence. Y-axes beyond 5 are truncated for better illustration of small values."}
...
```{r ci-example-measles-nb, echo = FALSE, fig.height=7, fig.width=6.5, fig.cap = "Example measles epidemics with negative binomial incidence. Y-axes beyond 5 are truncated for a better illustration of small values."}
fig_res_full_nb_measles <- res_dat_long %>%
  ...
  scale_colour_manual(values = cbPalette[-5]) +
  scale_fill_manual(values = cbPalette[-5]) +
  labs(y = "Rt estimates for measles epidemics with negative binomial incidence", x = "Time") + 
  scale_x_continuous(expand = expansion(c(0, 0.05))) + 
  scale_y_continuous(expand = expansion(c(0, 0.05))) + 
...
```{r ci-example-sars-pois, echo = FALSE, fig.height=7, fig.width=6.5, fig.cap = "Example SARS epidemics with Poisson incidence. Y-axes beyond 5 are truncated for a better illustration of small values."}
fig_res_full_pois_sars <- res_dat_long %>%
  ...
  scale_colour_manual(values = cbPalette[-5]) +
  scale_fill_manual(values = cbPalette[-5]) +
  labs(y = "Rt estimates for SARS epidemics with Poisson incidence", x = "Time") + 
  scale_x_continuous(expand = expansion(c(0, 0.05))) + 
  scale_y_continuous(expand = expansion(c(0, 0.05))) + 
  coord_cartesian(ylim = c(0, 5)) + 
  theme_bw() +
...
```{r ci-example-sars-nb, echo = FALSE, fig.height=7, fig.width=6.5, fig.cap = "Example SARS epidemics with negative binomial incidence. Y-axes beyond 5 are truncated for a better illustration of small values."}
fig_res_full_nb_sars <- res_dat_long %>%
  ...
  scale_colour_manual(values = cbPalette[-5]) +
  scale_fill_manual(values = cbPalette[-5]) +
  labs(y = "Rt estimates for SARS epidemics with negative binomial incidence", x = "Time") + 
  scale_x_continuous(expand = expansion(c(0, 0.05))) + 
  scale_y_continuous(expand = expansion(c(0, 0.05))) + 
  coord_cartesian(ylim = c(0, 5)) + 
  theme_bw() +
...
\clearpage
## Coverage comparisons of confidence intervals
...
kbl(df_ci_exp, booktabs = T, 
    caption = "Summary of experimental settings on coverage of confidence intervals",
    col.names = c("Length", "SI", "Rt scenario", "Incidence",
                  "SI for modelling", "Method")) %>%
  kable_styling(latex_options = c("repeat_header"))
```
...
1. percent coverage per coordinate (if available) across all synthetic data, 
2. percent overall coverage over all available coordinates, and
3. interval score [@Bracher_2021] averaged over all available coordinates.
...
for every time point, for example, \EpiEstim with a weekly sliding window 
does not provide estimates for the first week. The second metric aggregates across
all time and replications. 
The third metric, interval score [@Bracher_2021], is defined as
...
\clearpage
## Experimental results on interval coverage

Figs \@ref(fig:ci-coverage-measles) and \@ref(fig:ci-coverage-sars) displays 
...
\EpiLPS does the best here with averge coverage over all timepoints close to 1 (higher than nominal). 
This is consistent with the findings in the accuracy comparison (using KL values) 
and the illustration of sample epidemics in Figs
\@ref(fig:ci-example-measles-pois)--\@ref(fig:ci-example-sars-nb), where 
\EpiLPS is the most accurate. 
Using \RtEstim with $k=1,2,3$ has 100\% coverage at most timepoints except the
...
On the other hand, \RtEstim with $k=0$ tends to produce overly narrow intervals, leading to lower coverage. 
\EpiEstim with weekly sliding windows fails to cover the true 
...
\EpiEstim with monthly sliding windows has low percent coverage 
...
narrow intervals. \EpiFilter has lower percent coverage
...
CI_result <- readRDS(here::here("dat/CI_result_reduced_all.RDS"))
CI_result <- CI_result %>%
...
```{r ci-coverage-measles, echo=FALSE, fig.cap="Percent coverage of CIs per coordinate across 50 synthetic measles epidemics.", fig.height=5, fig.width=6.5, message=FALSE, warning=FALSE}
Rt_res_ci <- CI_result %>%
...
# replace NAs with negative values (in EpiEstim at the first few timepoints)
...
ci_cover_measles <- Rt_res_ci_noNAs %>% # exclude NAs
  filter(si_type == "measles") %>%
  ggplot(aes(y = ci_coverage, x = time_index)) +
  geom_line(aes(col = method)) + 
  facet_grid(dist ~ Rt_case, scales = "free") +
  scale_colour_manual(values = cbPalette[-5]) + 
  labs(y = "CI coverage for measles epidemics", x = "Time") + 
  theme_bw() +
  theme(legend.position = "bottom") +
  guides(color = guide_legend(title = NULL, byrow=TRUE))
...
```{r ci-coverage-sars, echo=FALSE, fig.cap="Percent coverage of CIs per coordinate across 50 synthetic SARS epidemics.", fig.height=5, fig.width=6.5, message=FALSE, warning=FALSE}
ci_cover_sars <- Rt_res_ci_noNAs %>% 
  filter(si_type == "SARS") %>% 
  ggplot(aes(y = ci_coverage, x = time_index)) +
  geom_line(aes(col = method)) + 
  facet_grid(dist ~ Rt_case, scales = "free") +
  scale_colour_manual(values = cbPalette[-5]) + 
  labs(y = "CI coverage for SARS epidemics", x = "Time") + 
  theme_bw() +
  theme(legend.position = "bottom") +
  guides(color = guide_legend(title = NULL, byrow=TRUE))
print(ci_cover_sars)
...
Figs \@ref(fig:ci-percentage-measles) and \@ref(fig:ci-percentage-sars) 
display the percent coverage of 95% CIs averaged over all timepoints and
50 random replications of \meas and \sars epidemics respectively. 
...
\EpiFilter has better coverage under Poisson 
...
\EpiLPS is the closest to nominal in most cases, and even
in the hardest problem, its empirical coverage is quite accurate. 


```{r ci-percentage-measles, echo=FALSE, fig.cap="Percent coverage of CIs over all timepoints for 50 synthetic measles epidemics. The orange dashed line represents 95\\% percentage of coverage across all timepoints.", fig.height=4.5, fig.width=6.2, message=FALSE, warning=FALSE}
ci_percent_measles <- CI_result %>% 
  ...
  labs(x = "", y = "CI percentages of coverage for measles epidemics") + 
  theme_bw() + 
  theme(axis.text.x = element_blank(), legend.position = "bottom",
        axis.ticks.x = element_blank()) +
  guides(color = guide_legend(title = NULL, byrow=TRUE))
...
```{r ci-percentage-sars, echo=FALSE, fig.cap="Percent coverage of CIs over all timepoints for 50 synthetic SARS epidemics. The orange dashed line represents 95\\% percentage of coverage across all timepoints.", fig.height=4.5, fig.width=6.2, message=FALSE, warning=FALSE}
ci_percent_sars <- CI_result %>% 
  ...
  scale_colour_manual(values = cbPalette[-5]) +
  geom_hline(yintercept = 0.95, color = "orange", linetype = "dashed") + 
  labs(x = "", y = "CI percentages of coverage for SARS epidemics") + 
  theme_bw() + 
  theme(axis.text.x = element_blank(), legend.position = "bottom",
        axis.ticks.x = element_blank()) +
  guides(color = guide_legend(title = NULL, byrow=TRUE))
...
Figs \@ref(fig:ci-score-measles) and \@ref(fig:ci-score-sars) display the 
interval scores of the 95% CIs averaged over 50 random \meas and \sars epidemics 
respectively. Our method always has the lowest or close to the lowest interval
scores. For Poisson, \EpiFilter has the lowest interval scores, 
and the scores of \RtEstim are slightly higher. \EpiLPS has very large interval 
...
```{r ci-score-measles, echo=FALSE, fig.cap="Interval scores averaged over all coordinates for 50 synthetic measles epidemics.", fig.height=6, fig.width=6.2, message=FALSE, warning=FALSE}
ci_score_measles <- CI_result %>% 
  ...
  scale_y_log10(breaks = c(0.1, 1, 10, 10000, 1e7, 1e10), labels = c(0.1, 1, 10,  expression(10^4),expression(10^7),expression(10^10))) +
  theme_bw() + 
  labs(y = "Interval scores for measles epidemics", x = "") + 
  theme(axis.text.x = element_blank(), legend.position = "bottom",
        axis.ticks.x = element_blank()) +
  guides(color = guide_legend(title = NULL, byrow=TRUE))
...
```{r ci-score-sars, echo=FALSE, fig.cap="Interval scores averaged over all coordinates for 50 synthetic SARS epidemics.", fig.height=4.5, fig.width=6.2, message=FALSE, warning=FALSE}
ci_score_sars <- CI_result %>% 
  ...
  scale_y_log10() +
  theme_bw() + 
  labs(y = "Interval scores for SARS epidemics", x = "") + 
  theme(axis.text.x = element_blank(), legend.position = "bottom",
        axis.ticks.x = element_blank()) +
  guides(color = guide_legend(title = NULL, byrow=TRUE))
print(ci_score_sars)
...
Figs 5 and 6 in the manuscript. A full visualization of each case is provided 
in Section \@ref(sec:samples). 
Here, we provide the condensed visualization of the other cases in Figs 
...
```{r display-example-pois-sars, echo=FALSE, fig.height=4, fig.width=6.2, fig.cap = "Example of instantaneous reproduction number estimates for SARS epidemics with Poisson observations. Y-axes beyond 5 are truncated for a better illustration of small values."}
...
```{r  display-example-nb-meas, echo=FALSE, fig.height=4, fig.width=6.2, fig.cap = "Example of instantaneous reproduction number estimates for measles epidemics with negative binomial observations. Y-axes beyond 5 are truncated for a better illustration of small values."}
...
\clearpage
## Alternative view of the difference between fitted and true Rt estimates

We also provide an alternative view of Figs 5 \& 6 in the manuscript by 
plotting $\calR_t - \hat{\calR}_t$ in Figs 
\@ref(fig:display-example-pois-meas-alter) and 
\@ref(fig:display-example-nb-sars-alter) respectively. 
Figs \@ref(fig:display-example-pois-sars-alter) and 
\@ref(fig:display-example-nb-meas-alter) provide the alternative view of Figs 
...
```{r display-example-pois-meas-alter, echo=FALSE, fig.height=4, fig.width=6.2, fig.cap = "Difference between the true and estimated instantaneous reproduction numbers for measles epidemics with Poisson observations. Y-axes beyond 1.5 are truncated for a better illustration of small values."}
fig_res_pois_meas_alter <- res_dat_long %>%
  ...
  facet_wrap( ~ Rt_case, scales = "free") +
  scale_colour_manual(values = cbPalette[-5]) +
  labs(y = expression(R[t]-widehat(R)[t]), x = "Time") + 
  scale_x_continuous(expand = expansion(c(0, 0.05))) + 
  scale_y_continuous(expand = expansion(c(0, 0.05))) + 
...
```{r display-example-nb-sars-alter, echo=FALSE, fig.height=4, fig.width=6.2, fig.cap = "Difference between the true instantaneous reproduction number and its estimation for SARS epidemics with negative binomial observations. Y-axes beyond 3 are truncated for a better illustration of small values."}
...
```{r display-example-pois-sars-alter, echo=FALSE, fig.height=4, fig.width=6.2, fig.cap = "Difference between the true and estimated instantaneous reproduction numbers for SARS epidemics with Poisson observations. Y-axes beyond 1.5 are truncated for a better illustration of small values."}
fig_res_pois_meas_alter <- res_dat_long %>%
  filter(si_type == "SARS", dist == "Poisson") %>% 
  mutate(dRt = Rt_value - trueRt) %>%
  group_by(Rt_case, method) %>%
...
```{r display-example-nb-meas-alter, echo=FALSE, fig.height=4, fig.width=6.2, fig.cap = "Difference between the true and estimated instantaneous reproduction numbers for measles epidemics with negative binomial observations. Y-axes beyond 1.5 are truncated for a better illustration of small values."}
fig_res_nb_sars_alter <- res_dat_long %>%
  filter(si_type == "measles", dist == "Negative Binomial") %>% 
  mutate(dRt = Rt_value - trueRt) %>%
  group_by(Rt_case, method) %>%
...
in Fig \@ref(fig:plot-cancovid-ci). An alternative display which plots all estimated 
curves in one panel for an easier comparison is provided in Fig 
...
the early stage of the epidemic. \EpiFilter is much more wiggly than other estimates.
...
dat_cancovid_fitted_long %>%
  ...
  scale_colour_manual(values = cbPalette[-5]) +
  scale_fill_manual(values = cbPalette[-5]) +
  labs(x = "Year", y = "Rt estimates") +
  theme_bw() +
  theme(legend.position = "bottom", strip.text = element_blank(),  panel.spacing = unit(0.4, "lines")) +
  guides(color = guide_legend(title = NULL, byrow=TRUE), fill = guide_legend(title = NULL, byrow=TRUE))
```

```{r plot-cancovid-one, echo=FALSE, fig.height=3.5, fig.width=6.2, fig.cap = "Rt estimates for Covid19. Y-axis beyond 3 is truncated for a better display of the fluctuation in small values. EpiFilter is excluded here, because its estimates are too wiggly and make the plot less readable."}
dat_cancovid_fitted_long %>%
...
We also apply all methods on Flu in 1918. The results are visualized in Figs 
\@ref(fig:plot-flu-ci) and \@ref(fig:plot-flu-one). 
\EpiEstim with weekly sliding windows, \EpiFilter and \RtEstim $(k=0)$ capture 
...
```{r plot-flu-ci,  echo=FALSE, fig.height=8, fig.width=6.2, fig.cap = "Rt estimates with CIs for Flu 1918. Y-axes are truncated beyond 3.5 for a better display of the fluctuation in small values."}
...
dat_flu_fitted_long %>%
  ...
  scale_colour_manual(values = cbPalette[-5]) +
  scale_fill_manual(values = cbPalette[-5]) +
  labs(x = "Days since start of epidemic", y = "Rt estimates") +
  theme_bw() +
  theme(legend.position = "bottom", strip.text = element_blank(),  panel.spacing = unit(0.4, "lines")) +
  guides(color = guide_legend(title = NULL, byrow=TRUE), fill = guide_legend(title = NULL, byrow=TRUE))
```

```{r plot-flu-one,  echo=FALSE, fig.height=3.5, fig.width=6.2, fig.cap = "Rt estimates for Flu 1918. Y-axis beyond 3.5 is truncated for a better display of the fluctuation in small values."}
dat_flu_fitted_long %>%
  filter(fitted_Rt > 0) %>% # remove NAs marked by -1
  group_by(Method) %>%
  ggplot(aes(x = Date)) + 
...
ggsave(here::here("fig/Fig1.png"), plot = CANCovid_fig, width = 6.5, height = 6.5)
ggsave(here::here("fig/Fig1.tiff"), plot = CANCovid_fig, width = 6.5, height = 6.5)
```
...
fig_sample <- dat_example_long %>%
  ...
  scale_color_brewer(palette = "Set1") +
  theme_bw() +
  theme(legend.position = "bottom", text = element_text(family = "sans", size = 12)) +
  guides(color = guide_legend(title = NULL))
ggsave(here::here("fig/Fig2.png"), fig_sample, width = 7.5, height = 6)
ggsave(here::here("fig/Fig2.tiff"), fig_sample, width = 7.5, height = 6)
```

```{r Figs3+4-display-simulation-results, eval = FALSE, include = FALSE}
# exclude the extremely large KL values (excluding the first week)
Rt_res_long_week_no_outliers <- Rt_result %>% 
  filter(len == 300, method != "EpiEstim (monthly)") %>% 
  select(Rt_case, dist, method, Rt_kl, si_type, si_pars) %>% 
...
fig_kl_week_measles <- Rt_res_long_week_no_outliers %>% 
  ...
  scale_colour_manual(values = cbPalette[c(1,3,4,6:9)]) +
  labs(x = "", y = "KL divergence for measles epidemics") +
  theme_bw() +
  theme(axis.text.x = element_blank(), legend.position = "bottom",
        axis.ticks.x = element_blank()) +
...
fig_kl_week_sars <- Rt_res_long_week_no_outliers %>% 
  ...
  scale_y_log10() +
  scale_colour_manual(values = cbPalette[c(1,3,4,6:9)]) +
  labs(x = "", y = "KL divergence for SARS epidemics") +
  theme_bw() +
  theme(axis.text.x = element_blank(), legend.position = "bottom",
        axis.ticks.x = element_blank()) +
  theme(text = element_text(family = "sans", size = 12)) +
...
ggsave(plot=fig_kl_week_measles, here::here("fig/Fig3.png"), width = 6.5, height = 5)
ggsave(plot=fig_kl_week_sars, here::here("fig/Fig4.png"), width = 6.5, height = 5)
ggsave(plot=fig_kl_week_measles, here::here("fig/Fig3.tiff"), width = 6.5, height = 5)
ggsave(plot=fig_kl_week_sars, here::here("fig/Fig4.tiff"), width = 6.5, height = 5)
```
...
ggsave(plot=fig_res_pois_measles, here::here("fig/Fig5.png"), width = 6.5, height = 5)
ggsave(plot=fig_res_NB_sars, here::here("fig/Fig6.png"), width = 6.5, height = 5)
ggsave(plot=fig_res_pois_measles, here::here("fig/Fig5.tiff"), width = 6.5, height = 5)
ggsave(plot=fig_res_NB_sars, here::here("fig/Fig6.tiff"), width = 6.5, height = 5)
```
...
fig_covid_Rt <- df_covid_Rt %>%
  ...
  labs(y ="Rt estimates for Covid-19 pandemic in Canada", x = "Month", color = "\u03BB") +
  ...
  scale_x_date(date_breaks = "8 months", date_labels = "%b %Y", expand = expansion(c(0, 0.05))) +
  scale_y_continuous(expand = expansion(c(0, 0.05))) + 
  coord_cartesian(ylim = c(0, 2)) + 
...
ggsave(here::here("fig/Fig7.png"), CanCovid_Rt_all, width = 7.5, height = 6)
ggsave(here::here("fig/Fig7.tiff"), CanCovid_Rt_all, width = 7.5, height = 6)
```
...
ggsave(here::here("fig/Fig8.png"), Flu_case, width = 6.5, height = 5)
ggsave(here::here("fig/Fig8.tiff"), Flu_case, width = 6.5, height = 5)
```
...
ggsave(here::here("fig/Fig9.png"), Flu_Rt, width = 6.5, height = 5)
ggsave(here::here("fig/Fig9.tiff"), Flu_Rt, width = 6.5, height = 5)
```
