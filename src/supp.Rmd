...
fit_covidBC_RtEstim0 <- cv_estimate_rt(
  x = cancovid$date, cancovid$incident_cases, korder = 0, nfold = 10, maxiter = 1e7L, 
...
fit_covidBC_RtEstim1 <- cv_estimate_rt(
  x = cancovid$date, cancovid$incident_cases, korder = 1, nfold = 10, maxiter = 1e7L, 
  nsol = 50, error_measure = "deviance", lambda_min_ratio = 1e-6, delay_distn = covid_delay_distn)
ci_covidBC_RtEstim1 <- confband(fit_covidBC_RtEstim1, "lambda.min", 1-alpha)

fit_covidBC_RtEstim2 <- cv_estimate_rt(
  x = cancovid$date, cancovid$incident_cases, korder = 2, nfold = 10, maxiter = 1e7L, 
  nsol = 50, error_measure = "deviance", lambda_min_ratio = 1e-6, delay_distn = covid_delay_distn)
ci_covidBC_RtEstim2 <- confband(fit_covidBC_RtEstim2, "lambda.min", 1-alpha)

fit_covidBC_RtEstim3 <- cv_estimate_rt(
  x = cancovid$date, cancovid$incident_cases, korder = 3, nfold = 10, maxiter = 1e7L, 
  nsol = 50, error_measure = "deviance", lambda_min_ratio = 1e-6, delay_distn = covid_delay_distn)
...
# fit model
fit_week <- EpiEstim::estimate_R(incid = cancovid$incident_cases, config = config_week, 
...
# fit model
fit_month <- EpiEstim::estimate_R(incid = cancovid$incident_cases, config = config_month, 
                                  method = "non_parametric_si")
...
# EpiLPS
fitLPS <- EpiLPS::estimR(incidence = cancovid$incident_cases, si = covid_delay_distn, K = 40, 
                         dates = cancovid$date, CoriR = FALSE)
...
w <- delay_calculator(cancovid$incident_cases, x = cancovid$date, delay_distn = covid_delay_distn)
R_filter <- epiFilter(Rgrid, m, eta=0.1, pR0, len, w, cancovid$incident_cases, a = alpha)
...
start_dates <- data.table(rt_ci_covid, variant = can_pred_class$var[-1:-65], date = cancovid$date) %>% group_by(variant) %>% summarize(min(date))
...
cv_mod_covid1 <- cv_estimate_rt(
  x = cancovid$date, cancovid$incident_cases, korder = 1, nfold = 10,
  maxiter = 1e7L, nsol = 50, error_measure = "deviance", 
...
cv_mod_covid3 <- cv_estimate_rt(
  x = cancovid$date, cancovid$incident_cases, korder = 3, nfold = 10,
  maxiter = 1e7L, nsol = 50, error_measure = "deviance", 
  lambda_min_ratio = 1e-6, delay_distn = covid_delay_distn
)
