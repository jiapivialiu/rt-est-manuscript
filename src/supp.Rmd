...
Solve the following problem $\lambda_{max} := \sup_{\alpha\neq \boldsymbol{0}} \lambda$ through solving $\sup_{\alpha} \frac{\partial \mathcal{L}_{\lambda}(\alpha)}{\pm \partial \alpha_i} = \sup_{\alpha_i} D^{\dagger}\lr{ e^{D^{\dagger} \alpha} \eta - y} = \max_{i} {\lvert D^{\dagger} \lr{\eta - y}\rvert}_i$ as $D^{\dagger} \alpha \to \boldsymbol{0}$ for $i=1,\cdots, n$. 


## Cross validation of RtEstim in simulation 

We run 3-fold cross validation (CV) to choose the best tuning parameter
...
first and last entries) into three folds evenly and randomly, and build models on 
...
The full KL ratio (KL values over the baseline) for Poisson and negative 
Binomial incidences without excluding outliers are shown in the following figures. Both y-axes are in logarithmic scales for a better visualization, since there 
are extreme values. 
...
fig_kl_pois_full_week <- Rt_result %>%
  ...
  scale_y_log10(breaks = c(1,6,20,100,4000), 
                labels = c("1","6","20","100","4000")) + 
  scale_colour_manual(values = cbPalette) + 
  labs(y = "Averaged KL ratio for Poisson incidences", x="Rt Scenarios") +
  labs(color="Methods") +
  theme_bw()

fig_kl_nb_full_week <- Rt_result %>%
  filter(dist == "NB") %>%
  filter(method != "EpiEstim(month)") %>%
  mutate(Rt_kl = Rt_kl / KL_base) %>%
  mutate(method = fct_relevel(method, "EpiEstim(week)", "EpiLPS", 
...
fig_kl_week <- ggpubr::ggarrange(
  fig_kl_pois_full_week, fig_kl_nb_full_week, ncol=2, nrow=1, 
  common.legend = TRUE, legend = "bottom", 
  font.label = list(size = 14), align = "hv"
)
fig_kl_week
```
...
fig_kl_pois_full_month <- Rt_result %>%
  filter(method != "EpiEstim(week)") %>% 
  filter(dist == "poisson") %>%
  select(Rt_case, method, Rt_kl_month, KL_base_month) %>% 
  mutate(Rt_kl = Rt_kl_month / KL_base_month) %>%
  mutate(method = fct_relevel(method, "EpiEstim(month)", "EpiLPS", 
                              "RtEstim(k=0)", "RtEstim(k=1)", 
                              "RtEstim(k=3)")) %>% 
  group_by(Rt_case, method) %>%
  ...
  geom_boxplot(aes(col = method)) + 
  geom_hline(yintercept = 1, linetype = "dashed", color = "darkgrey") +
  scale_y_log10(breaks = c(1,6,20,100,1000), 
                labels = c("1","6","20","100","1000")) + 
  scale_colour_manual(values = cbPalette) + 
  labs(y = "Averaged KL ratio for Poisson incidences", x="Rt Scenarios") +
  labs(color = "Methods") + 
  theme_bw() 

fig_kl_nb_full_month <- Rt_result %>%
  filter(dist == "NB") %>%
  filter(method != "EpiEstim(week)") %>% 
  select(Rt_case, method, Rt_kl_month, KL_base_month) %>% 
  mutate(Rt_kl = Rt_kl_month / KL_base_month) %>%
  ...
  geom_boxplot(aes(col = method)) + 
  geom_hline(yintercept = 1, linetype = "dashed", color = "darkgrey") +
  scale_y_log10(breaks = c(1,6,50,200,1000), 
               labels = c("1","6","50","200","1000"))+
  scale_colour_manual(values = cbPalette) + 
  labs(y = "Averaged KL ratio for NB incidences", x="Rt Scenarios") +
  labs(color="Methods") +
  theme_bw()

fig_kl_month <- ggpubr::ggarrange(
  fig_kl_pois_full_month, fig_kl_nb_full_month, ncol=2, nrow=1, 
  common.legend = TRUE, legend = "bottom", 
...
## Synthetic effective reproduction numbers and Poisson incidences examples
...
of Gamma distribution, can significantly influence the peak values of incidences 
...
### Rt estimates for Poisson incidences using RtEstim
...
plot(rtestim_mod22)
cv_mod22 <- cv_estimate_rt(incidence2, korder=1, nfold=3, nsol=50, maxiter=1e7L, 
                          dist_gamma = gamma_pars2, error_measure = "deviance")
plot(cv_mod22)
...
### Rt estimates for Poisson incidences using EpiEstim
...
### Rt estimates for Poisson incidences using EpiLPS
...
## Synthetic effective reproduction numbers and negative Binomial incidences examples
...
### Rt estimates for negative Binomial incidences using RtEstim 

Estimate Rt using our `RtEstim` with the same settings as for Poisson incidences.

```{r Rt-NB-RtEstim, echo=FALSE, fig.width=3.5, fig.height=3}
mod_nb1 <- estimate_rt(nb_incidence1, korder=0, nsol=50, dist_gamma = c(3, 3))
plot(mod_nb1)
cv_mod_nb1 <- cv_estimate_rt(nb_incidence1, korder=0, nfold=30, nsol=50, 
...
plot(mod_nb22)
cv_mod_nb22 <- cv_estimate_rt(nb_incidence2, korder=1, nfold=3, nsol=50, 
                             dist_gamma = c(2.5, 2.5), error_measure = "deviance")
plot(cv_mod_nb22)
...
plot(mod_nb3)
cv_mod_nb3 <- cv_estimate_rt(nb_incidence3, korder=1, nfold=3, nsol=100, 
...
### Rt estimates for negative Binomial incidences using EpiEstim 

Estimate Rt  for negative Binomial incidences using `EpiEstim` with weekly sliding windows.
...
Estimate Rt for negative Binomial incidences using `EpiEstim` with monthly sliding windows.
...
### Rt estimates for negative Binomial incidences using EpiLPS  
...
Display results of Poisson incidences. 

```{r Pois-res-display, echo=FALSE}
...
ggsave(here::here("fig/Pois-res-plot.png"), fig_Pois_res, width = 7.81, height = 5.03)
```

Display results of negative Binomial incidences.

```{r NB-res-display, echo=FALSE}
fig21 <- res_dat %>%
...
ggsave(here::here("fig/NB-res-plot.png"), fig_NB_res, width = 7.81, height = 5.03)
```
...
save_example <- function(res_dat, palette = cbPalette){
  ...
  fig2 <- res_dat %>%
    ...
    geom_line() +
    #scale_y_log10() + # axis of incidence in log scale
    labs(title = "Poisson incidences", y=" ", x="") + #(in log scale) 
  ...
  fig3 <- res_dat %>%
    ...
    geom_line() +
    #scale_y_log10() + # axis of incidence in log scale
    labs(title = "NB incidences", y=" ", x="") + #(in log scale) 
    coord_cartesian(ylim = c(0, 2500)) +
    theme(plot.title = element_text(hjust = 0.5)) + 
    theme_bw()
