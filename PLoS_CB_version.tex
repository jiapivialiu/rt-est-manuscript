...
incidence. Ho et al.~\cite{ho2023accounting} estimated $\calR_t$ while
...
accessible in a lightweight \R\ package called \texttt{rtestim}.
\begin{figure}[!t]
  \centering
...
out, and then predict the held-out observations. Alternatively, one could use
...
of these synthetic experiments, Fig A.2.1 in \nameref{S1_supp} displays the
...
misspecification of SI distributions are given in Sections A.2.1 and A.3 in \nameref{S1_supp} respectively. 
...
\begin{figure}[!t]
  ...
  synthetic SARS incidence. The 4 $\calR_t$ scenarios are shown in the
  columns.} 
  \label{fig:samples}
\end{figure}
...
declines. We used weekly and monthly sliding windows, however, since neither
considerably outperforms the other across all scenarios, we defer the monthly
results to Section A.3.1 in \nameref{S1_supp}. \EpiLPS\ is another Bayesian approach
...
experimental study. \autoref{fig:kl-res-measles} visualizes the KL divergence 
across the seven methods with measles epidemics, and \autoref{fig:kl-res-sars}
visualizes the counterparts with SARS epidemics. For low incidence in
...
\begin{figure}[!t]
  ...
  sake of comparison with full range of the $y$-axis deferred to Fig A.3.1 in
  \nameref{S1_supp}.} 
...
\begin{figure}[!t]
  ...
  full range of the $y$-axis deferred to Fig A.3.1 in \nameref{S1_supp}.} 
...
estimated $\calR_t$ curve displayed in a separate panel is provided in Fig
A.6.1 in \nameref{S1_supp}. Ignoring the start of the epidemics, all methods
...
\begin{figure}[!t]
  ...
  \caption{{\bf $\calR_t$ estimates for realizations of a measles epidemic with
    ...
    $\calR_t$ curve displayed in a separate panel is provided in Fig A.6.1 in
    \nameref{S1_supp}.}
...
displayed in a separate panel is provided in Fig A.6.4 in \nameref{S1_supp}.
...
\begin{figure}[!t]
  ...
  \caption{
    ...
    $\calR_t$ curve displayed in a separate panel is provided in Fig A.6.4 in
    \nameref{S1_supp}.}
...
in \autoref{sec:intro}, where the dominant circulating variants are based on a
multinomial logistic regression model with variant probabilities from
\cite{duotang_2023} and the corresponding serial interval distributions are 
based on results from \cite{xu2023assessing}. Unlike in \autoref{fig:intro-fig}, where 
we consider a time-varying serial interval distribution, in this section we use the weighted average of the 
serial interval distributions for the four dominant variants for the purposes of comparison with other methods, none
...
deferred to Figs A.8.1 and A.8.2 in \nameref{S1_supp}. 
...
than one. Results from alternative software is deferred to Figs A.8.3 and 
A.8.4 in \nameref{S1_supp}.
...
immunity \cite{nash2023estimating}. One issue regarding the serial interval
...
\begin{thebibliography}{10}
  ...
  CAMEO~Group CVRRN. CoVaRR-NET/duotang: Release for Zenodo Archive; 2023.
...
{\bf Supplement for ``rtestim: Time-varying reproduction number estimation with
trend filtering''} 

This includes eight sections, Sections A.1--A.8. Section A.1: Derivation of Kullback Leibler divergence.
Section A.2: Additional details on experimental settings.
Section A.3: Additional accuracy comparisons.
Section A.4: Experimental results under misspecification of the serial interval distributions.
